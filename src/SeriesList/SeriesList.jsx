...
export const SeriesList = () => {
  ...
  // Initialize currentPage, currentNumberLimit, and maxNumberLimit
    ...
    const [currentNumberLimit] = useState(initialCurrentNumberLimit);
    ...
    // const [currentNumberLimit, setCurrentNumberLimit] = useState(3);
    ...
    let handleClickLastPage = (event) => {
        handleClick(event);
        setMaxNumberLimit(localStorage.value = pages.length+1);
        setMinNumberLimit(localStorage.value = (pages.length+1)-3);
    };
    ...
    return (
        <div className="main">
            ...
            <div className="navBarContainer">
                ...
                <div className="navBarContainerSec">
                        <div className="movieChangeBtnNBox">
                            <div className="h1NIcon">
                                <h1>Series</h1>
                                <div>
                                    <a className="movieChangeBtn" onClick={() => showV_()} ref={movieChangeBoxRef}><img src="https://icons.veryicon.com/png/o/internet--web/industrial-icon/sort-2.png"/></a>
                                </div>
                            </div>
                            <div className="movieChangeBox">
                                <a className="v_" href="/" onClick={home}><h1>Movies</h1></a>
                                <a className="v_" href="/animationList" onClick={home}><h1>Animations</h1></a>
                            </div>
                        </div>
                        <div className="searchInputContainer">{<Search/>}</div>
                        <div className="navBarRight">{<NavBarRight />}</div>
                </div>
            </div>
            <br/>
                ...
                {currentItem.map((movie) => {
                    return (
                     // <a href={`/singleMovie/${movie.id}`}  key={movie.id} target="_blank" rel="noopener noreferrer">
                        <a href={`/singleSeries/${movie.id}`}  key={movie.id} >
